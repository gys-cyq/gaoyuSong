		...
		$.ajax({
			...
			success: function(data) {
				...
				layui.each(data.data, (index, item) => {
                    ...
                    html +=
                        "<div class='item' style='padding: 3px 10px;cursor: pointer;' onmouseenter='getFocus(this)' data-item='" + JSON.stringify(item) + "'>" +
							item[opt.key] + "</div>";
					//}
				});
				if (html != "") {
                ...
                $("#" + sessionStorage.getItem("inputId")).next().find('div.item').unbind('click').bind('click', function () {
                    let json = $(this).attr('data-item');
                    window.getCon(this)
                    opt.onselect ? opt.onselect(JSON.parse(json)) : null;
                })
			}
		});
	}
